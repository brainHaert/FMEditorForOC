...
- (void)saveImage
{
    ...
    UIImage *shuiyinImg = [UIImage imageNamed:@"details_keep_icon"];
    UIImage *resultImage = [self addImage:image addMsakImage:shuiyinImg msakRect:CGRectMake(4, image.size.height-image.size.width/10, image.size.width/12, image.size.width/12)];
...
- (UIImage *)addImage:(UIImage *)useImage addMsakImage:(UIImage *)maskImage msakRect:(CGRect)rect
{
    ...
    UIImage *saveImage = [self bacImage:resultingImage AddText:[NSString stringWithFormat:@"By %@.",self.userName]];
    return saveImage;
}
#pragma mark 加文字水印
-(UIImage *)bacImage:(UIImage *)img AddText:(NSString *)text
{
    UIImageView *view = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, img.size.width, img.size.height)];
    view.image = img;
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0,0,0,0)];
    [label setNumberOfLines:0];
    label.frame = CGRectMake(img.size.width/9,img.size.height-img.size.width/13, img.size.width/2, img.size.width/27);
    label.text = text;
    label.textColor = [UIColor whiteColor];
    CGFloat height = 1;
    while (label.frame.size.height > height) {
        height++;
        CGSize size = [self sizeWithText:text font:[UIFont systemFontOfSize:height] maxSize:CGSizeMake(label.frame.size.width, MAXFLOAT)];
        height = size.height;
    }
    //水印上文字的大小
    label.font = [UIFont systemFontOfSize:height-5];
...
{
    CGSize sss = view.bounds.size;
    UIGraphicsBeginImageContextWithOptions(sss, NO, [UIScreen mainScreen].scale);
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:UICollectionView.class]) {
        ...
        if (self.currentImageArray.count > currentIndex) {
            currentIndex = [[self.currentImageArray objectAtIndex:currentIndex]integerValue];
        }
...
- (void)showFirstImage
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:UICollectionView.class]) {
        ...
        NSInteger item = self.currentImageIndex;
        if (self.currentImageArray.count > self.currentImageIndex) {
            item = [[self.currentImageArray objectAtIndex:self.currentImageIndex]integerValue];
        }
